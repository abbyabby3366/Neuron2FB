...
async function autoBetSbo(page, betEvent, referenceAcc, successBetListKey, fb2ConfigId) {
  ...
  try {
    ...
    // Check if ticketed odds are in opposite directions (one positive, one negative)
    const hasOppositeDirection =
      (ticketDataTarget.unconvertedTickettedOdds > 0 &&
        referenceUnconvertedTickettedOdds < 0) ||
      (ticketDataTarget.unconvertedTickettedOdds < 0 &&
        referenceUnconvertedTickettedOdds > 0);

    if (!hasOppositeDirection) {
      console.log(
        `${acc} - Ticketed odds are NOT in opposite directions. Target: ${ticketDataTarget.tickettedOdds}, Reference: ${referenceUnconvertedTickettedOdds}. Not within range, skipping.`,
      );
      let betFailedTime = new Date();
      betEvent.betFailedTime = betFailedTime;
      ...
      await writeData("tempFailBetList", betEvent);
      throw new Error(
        `${acc} - Ticketed odds not in opposite directions. Target: ${ticketDataTarget.tickettedOdds}, Reference: ${referenceUnconvertedTickettedOdds}`,
      );
    }
    ...
    let tickettedOvervalue =
      ...
      referenceUnconvertedTickettedOdds;

    console.log("FINAL EV:", tickettedOvervalue);
    ...
    console.log("waitbetSBO done", betPlacedTime);
