...
const brain = async (acc) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      // Filter tempFailBetListIBC
      let tempFailBetListIBC_ticket3838 = tempFailBetListIBC.filter(
        (entry) =>
          entry.betFailedReason === "ticket3838" &&
          currentTime - new Date(entry.betFailedTime) <
            brainParams.timeBetweenTicketFail * 1000,
      );
      let tempFailBetListIBC_ticketIBC = tempFailBetListIBC.filter(
        (entry) =>
          entry.betFailedReason === "ticketIBC" &&
          currentTime - new Date(entry.betFailedTime) <
            brainParams.timeBetweenTicketFail * 1000,
      );
      let tempFailBetListIBC_ev = tempFailBetListIBC.filter(
        (entry) =>
      ...
      data_Ibc = data_Ibc.filter((entry) => {
        // Check various conditions for filtering
        const isStaleData =
          currentTime - new Date(entry.timeScraped) >
          brainParams.dataStaleTimeInSeconds * 1000;
        const isFailedBet =
          tempFailBetListIBC_ticket3838.some((failBet) =>
            isSameBet(entry, failBet),
          ) ||
          tempFailBetListIBC_ticketIBC.some((failBet) =>
            isSameBet(entry, failBet),
          ) ||
          tempFailBetListIBC_ev.some((failBet) => isSameBet(entry, failBet));
