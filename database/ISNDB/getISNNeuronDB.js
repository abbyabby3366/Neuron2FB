...
function getISNNeuronDB() {
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      let ISNConfirmedArray = leagueNames.map((el) => el.leagueName);
      console.log("current ISN confirmed leagues: ", ISNConfirmedArray.length);

      fs.writeFile(
        "./ISNDB/output/ISNConfirmedLeagues.json",
